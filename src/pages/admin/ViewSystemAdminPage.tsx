...
import { UserCog, Mail, Phone, Calendar, Shield, ArrowLeft, Loader2, CreditCard, User as UserIcon } from 'lucide-react';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { toast } from 'sonner';
import { formatCPF, formatPhone } from '@/utils/formatters';

interface SystemAdminDetails extends User {
    cpf?: string | null;
...
export function ViewSystemAdminPage() {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const [admin, setAdmin] = useState<SystemAdminDetails | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const loadAdmin = async () => {
            if (!id) {
                toast.error('ID do administrador não fornecido');
                navigate('/admin');
                return;
            }
            ...
            try {
                setLoading(true);
                // Buscar da lista de admins (por enquanto, até criar endpoint específico)
                const admins = await adminService.listSystemAdmins({ limit: 1000 });
                ...
                const adminData: SystemAdminDetails = {
                    ...foundAdmin,
